...
// метод создания двумерного массива (матрицы)
    ...
    else
    {
        for (int i = 0; i < matrix.GetLength(0); i++)
        {
            for (int j = 0; j < matrix.GetLength(1); j++)
            {
                ...
                {
                    Console.WriteLine($"Элемент массива с индексами [{indexI},{indexJ}] = {matrix[i, j]}");
                }
            }
        }
    }
...
if (i > m - 1 || j > n - 1)
{
    Console.WriteLine($"Элемент в массиве с индексами [{i},{j}] отсутствует");
}
else
{
